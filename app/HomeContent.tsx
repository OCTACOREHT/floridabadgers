...
type HeroBackgroundSliderProps = {
  slides: (HeroSlide | { image_url: string })[];
  currentIndex: number;
};
  ...
  return (
    <div className="absolute inset-0 z-0">
      <AnimatePresence mode="wait">
        <motion.div
          key={currentIndex}
          initial={currentIndex === 0 ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 1.05 }}
...
export default function HomeContent({ 
  ...
  const [homeNewsCards, setHomeNewsCards] = useState<NewsArticle[]>(initialNews);
  ...
  useEffect(() => {
    async function loadInitialData() {
      try {
        const [fRes, nRes, hRes] = await Promise.all([
          fetch(`/api/upsl/fixtures?season=${SCHEDULE_SEASON}`),
          fetch("/api/news?limit=3"),
          fetch("/api/hero-slides"),
        ]);

        if (fRes.ok) {
          const fJson = (await fRes.json()) as FixturesApiResponse;
          if (fJson.fixtures) setFixtures(fJson.fixtures);
          if (fJson.error) setFixturesError(fJson.error);
        }

        // Re-fetch in background to sync any very fresh changes
        if (nRes.ok) {
          const nJson = (await nRes.json()) as { articles?: NewsArticle[] };
          if (nJson.articles) setHomeNewsCards(nJson.articles);
        }

        if (hRes.ok) {
          const hJson = (await hRes.json()) as HeroSlidesApiResponse;
          if (hJson.slides?.length) {
            setSlides(hJson.slides);
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    const totalSlides = slides.length || heroImages.length;
  ...
  const activeSlides = slides.length
    ? slides
    : heroImages.map((url) => ({ image_url: url, titre: null, sous_titre: null }));
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative h-[100svh] min-h-[600px] w-full overflow-hidden bg-slate-900 flex flex-col justify-center">
        {/* Background Slider */}
        <HeroBackgroundSlider slides={activeSlides} currentIndex={currentSlideIndex} />
        ...
        {/* Content */}
        <div className="relative z-20 max-w-[1320px] w-full mx-auto px-5 sm:px-6 xl:px-10 pt-48 pb-24">
          <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-10">
            ...
            <motion.div
              ...
              animate="show"
              variants={stagger}
              className="flex flex-col gap-4 sm:gap-6 max-w-2xl w-full mb-10 lg:mb-10"
            >
              <motion.div variants={fadeUp}>
                <SectionLabel><span className="invisible">UPSL Florida South Zone II</span></SectionLabel>
                <h1 className="text-4xl sm:text-4xl md:text-5xl xl:text-6xl font-black uppercase leading-[1.03] tracking-tight text-white drop-shadow-xl font-[family-name:var(--font-montserrat)]">
                  {currentSlide?.titre ? (
                    currentSlide.titre
                  ) : (
                    <>
              ...
              >
                {currentSlide?.sous_titre ? (
                  currentSlide.sous_titre
                ) : (
                  "Our mission is to provide a platform for talented and motivated young soccer players to grow, compete, and succeed - both on the field and in life."
                )}
              </motion.p>
              ...
              <motion.div variants={fadeUp} className="flex w-full justify-center sm:justify-start gap-4 sm:gap-8 pt-4 sm:pt-8 lg:mb-16">
                {[
                  ...
                  <div key={s.label} className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] sm:text-xs text-white/60 uppercase tracking-widest mt-1.5">{s.label}</div>
                    </div>
                  </div>
                ))}
              </motion.div>
            ...
            {/* Right - Match ticket (Positioned absolutely on large screens to keep it independent) */}
            <div className="w-full lg:w-auto lg:absolute lg:bottom-[136px] lg:right-5 xl:right-10 z-30">
              <motion.div
